...
describe("WB-11 /api/services 계약", () => {
  ...
  it("잘못된 limit 방어 (문자/음수 → 기본값, 과대 → clamp)", async () => {
    for (const bad of ["abc", "-5", "999999"]) {
      const res = await servicesGET(req(`/api/services?limit=${bad}`));
      expect(res.status).toBe(200);
      const body = await res.json();
  ...
  it("relevance CSV 파싱", async () => {
    const res = await servicesGET(req("/api/services?relevance=high"));
    const body = await res.json();
...
describe("WB-12 /api/services 검색", () => {
  it("?q=암 → 결과가 있고 전부 관련 필드에 검색어 포함", async () => {
    const res = await servicesGET(req("/api/services?q=%EC%95%94"));
    const body = await res.json();
...
describe("/api/regions", () => {
  it("시도 목록", async () => {
    const res = await regionsGET(req("/api/regions"));
    const body = await res.json();
    expect(body.sido).toContain("서울특별시");
  });

  it("시군구 목록 (sido 종속)", async () => {
    const res = await regionsGET(req("/api/regions?sido=%EC%84%9C%EC%9A%B8%ED%8A%B9%EB%B3%84%EC%8B%9C"));
    const body = await res.json();
